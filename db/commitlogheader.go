...
type CommitLogHeader struct {
	// 跟踪哪些列族在提交日志中被修改过。每个列族有一个标志位（true 表示该列族被修改过），通过该标志可以确定是否需要在恢复数据时重放该列族的日志。
	dirty *bitset.BitSet
...
// 将指定列族的 dirty 标志位置为 false，并将该列族的最后刷新位置重置为 0。
...
// 检查是否可以删除；如果有任何列族的数据还没有刷盘，则不能删除日志。
...
// 将 header 中的 dirty 位图和 lastFlushedAt 数组序列化存入 dos 中
