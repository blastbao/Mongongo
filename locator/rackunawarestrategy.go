...
func (rus *RackUnawareStrategy) getStorageEndPoints(token string, tokenEndPointMap map[string]network.EndPoint) []network.EndPoint {
	...
	// spew.Printf("tokenEndPointMap: %v\n", tokenEndPointMap)
	// spew.Printf("tokens: %v\n", tokens)
...
func (rus *RackUnawareStrategy) GetWriteStorageEndPoints(token string) map[network.EndPoint]bool {
	tokenToEndPointMap := rus.TokenMetadata.CloneTokenEndPointMap()
	bootstrapTokensToEndpointMap := rus.TokenMetadata.CloneBootstrapNodes()
	tokenList := rus.getStorageTokens(token, tokenToEndPointMap, bootstrapTokensToEndpointMap)
	list := make([]network.EndPoint, 0)
	for _, t := range tokenList {
		e, ok := tokenToEndPointMap[t]
		if ok == false {
			e = bootstrapTokensToEndpointMap[t]
		}
		list = append(list, e)
	}
	retrofitPorts(list)
